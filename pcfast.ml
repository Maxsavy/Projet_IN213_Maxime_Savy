...
let rec print oc e =
  ...
  | Pair (e1, e2) -> fprintf oc "(pair %a %a)" print e1 print e2
  | First e -> fprintf oc "(first %a)" print e
  | Second e -> fprintf oc "(second %a)" print e
;;

let rec wrap_with_lib lib user =
  match lib with
  | Let (x, e1, body) -> Let (x, e1, wrap_with_lib body user)
  | Letrec (x, e1, body) -> Letrec (x, e1, wrap_with_lib body user)
  | _ -> user
;;
